      ...
      .then(data => {
        ...
        fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=imperial&appid=${apiKey}`)
          ...
          .then(currentData => {
            ...
            const currentHumidity = currentData.main.humidity;// Define a constant for the API key to access OpenWeatherMap's API
            const apiKey = "17c217eb87e1e62fc400a04808b0be98";
                ...
                fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${cityName}&units=imperial&appid=${apiKey}`)
                  ...
                  .then(data => {
                    ...
                    for (let i = 1; i <= 5; i++) {
                      ...
                      // Extract Weather Emoji
                      const weatherIconCode = dayData.weather[0].icon;
                      const weatherEmoji = `<img src="http://openweathermap.org/img/wn/${weatherIconCode}.png">`; // Emoji image
                      ...
                      // Update one of the divs for the 5-day forecast
                      document.getElementById(`day${i + 1}`).innerHTML = `
                        Date: ${formattedDate} ${weatherEmoji}<br> // Date and Weather Emoji
                        Temp: ${temperature} F<br>
                        Wind: ${windSpeed} MPH<br>
                        Humidity: ${humidity}%
                      `;
